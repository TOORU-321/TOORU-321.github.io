...
APP_URL = "https://columns.l-mine.com/app/"      # 会員制の動画視聴Webアプリ「エルラボ＋」
TEMPLATE_FROM = 98                                # この番号以降のコラムに エルラボ＋ の案内を付与（オファーテンプレ）

# 記事末尾のオプトイン枠（メルマガ主体・n>=TEMPLATE_FROM でエルラボ＋の控えめな1行を追記）
def optin_footer(n):
    app_line = (f'\n      <p class="optin-sub">動画でじっくり学びたい方は、会員制の動画視聴アプリ「<a href="{APP_URL}" target="_blank" rel="noopener">エルラボ＋</a>」もご用意しています。</p>'
...
def popup_html(n):
    app_btn = (f'\n      <a class="lm-pop-app" href="{APP_URL}" target="_blank" rel="noopener">動画で学ぶ｜エルラボ＋</a>'
               if n >= TEMPLATE_FROM else '')
