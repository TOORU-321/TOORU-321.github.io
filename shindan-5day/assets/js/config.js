...
(function (global) {
  ...
  SC.config = {
    ...
    /* 1日にひとつのDAYだけ開く（2026-09-11 とーる判断）。
     * LINEのシナリオが「明日の朝、ご案内しますね」と約束しているので、
     * アプリもそれに合わせる。切りたくなったら false にする。
     * 判定そのものは logic/day-gate.js */
    oneDayPerDay: true,
     ...
     *
     * 公開物では出さない（下のとおり固定済み）。
     * 消さずに残してあるのは、作業用のファイルで使うため。 */
    devTools: function () {
      /* 公開用の書き出しで固定した（tools/build-publish.py）。
       * 作業用のファイルは元のままで、そちらでは今までどおり使える。
       * 公開物では、URLに何を付けても開発用の道具は出ない。
       *
       * ★これは公開画面から開発用の道具を外すためのもの。
       *   本人確認や、サーバーへの不正な要求を防ぐ対策ではない。 */
      return false;   /* 公開用に固定 */
    },
